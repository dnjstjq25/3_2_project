...
<?php
	...
	if($mode=="modify"){
		try{
			...
			$sql = "update dnjstjq25.greet1 set subject=?, content=? where num=?";
			...
			header("Location:http://dnjstjq25.dothome.co.kr/qna.php");
	...
	try{
		$pdo->beginTransaction();
		$sql = "insert into dnjstjq25.greet1(id,name,subject,content)";
		$sql .= "values(?, ?, ?, ?)";
		$stmh = $pdo->prepare($sql);
