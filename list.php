...
if($mode=="search"){
	...
	$sql="select * from dnjstjq25.greet where $find like '%search%' order by num desc";
} else {
	$sql="select * from dnjstjq25.greet order by num desc limit $first_num, $scale";
}

try{
	$stmh = $pdo->query($sql);

	$sql = "select * from dnjstjq25.greet";
	$stmh1 = $pdo->query($sql);
...
while($row = $stmh->fetch(PDO::FETCH_ASSOC)){
	...
	<div id="list_item">
		...
		<div id="list_item2"><a href="noti_view.php?num=<?=$item_num?>&page=<?=page?>"><?=$item_subject?></a></div>
	...
	<?php
	 ...
	 if($page!=1 && $page>$page_scale)
	 {
	 	...
	 	if($prev_page <= 0)
	 		$prev_page = 1;
	 	print "<a href=notice.php?page=$prev_page>◀ </a>";
	 }
	 ...
	 for($i=$start_page; $i<=$end_page && $i<= $total_page; $i++) 
      {        
        if($page==$i) 
           ...
           print "<a href=notice.php?page=$i>[$i]</a>";
      }
