...
//<![CDATA[
function incre(e){
 var cot=document.getElementsByName("count")[0];  //count 값을 가지고 온다
...
var won=document.getElementsByName("price")[0];  //input 판매가 에서 won값을 가지고 온다
var b=parseInt(won.value);  //won값에 b값을 넣는다

document.getElementById('totals').innerHTML=comma(parseInt(a*b)); //합계

  function comma(str) { //(콤마삽입)
       str = String(str); 
       return str.replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,'); 
    }
    document.getElementsByName("_count")[0].value=a;
    document.getElementsByName("totals")[0].value=a*b;
  } 
//]]> 
</script>
	<style type="text/css">
	...
	.menu-list-c:hover .sub-menu4{display: block;}
	 ...
	 #view_img{width: 500px; float:left;}
	 .view_content{width: 80%; float: left; margin-top: 20px;}
	 .view_subject{float: left; margin-left: 50px; margin-top: 20px;}
	 .buy_bt{float: right; margin-right: 30px;}
	 .buy_bt_s{border: none; width: 80px; height: 40px; background-color: black; color: #fff;}
	 .buy_bt_s:hover{text-decoration: none; animation: a 0.5s ease-in-out; -webkit-animation: a 0.5s ease-in-out; 
			color: #fff;}
...
<div class="menu-arrow"><a href="brand.php">브랜드</a> > 상품정보</div>
<div class = "wrap">
	<div class ="wrap2">
	<form name='buys' action='./buys.php' method='post' enctype='multipart/form-data'>		
        ...
        <div id="view_title">
	         <div id="view_img">
				<img src="./data/<?=$row[file01]?>" width="500px" height="500px"></a>
	         </div>
	         <div class="view_subject">
		  		* 상품명 : <?= $item_subject ?><br><br>
	         	* 가격 : ₩<?=number_format($row[price])?><br><br>
	         	* 수량 <input type='text' name='count' value='1' readonly style="width:35px; height:20px">
						<input type="button" value="▲" onclick='incre(1);'>
						<input type="button" value="▼" onclick='incre(-1);'>
						<input  type='hidden' name='_count' value='1'> <br><br><br><br>
				* 상 품 합 계: <strong><span id='totals'><?=number_format($row['price'])?></span></strong>&nbsp;원
		<input type='hidden' name='totals' value='<?=$row[price]?>'> <br><br>
			
	         	<div class="buy_bt">
	         		<input class="buy_bt_s" type="submit" name="buy" value="결 제 하 기">
	         	</div>
	         </div>
