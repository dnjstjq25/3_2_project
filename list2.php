...
<head>
	...
	<style>
		...
		#list_item #list_item5{	float:left;	margin-left: 30px; width:40px;	text-align:center;}
...
if($mode=="search"){
	...
	$sql="select * from dnjstjq25.mtm where $find like '%search%' order by num desc";
} else {
	$sql="select * from dnjstjq25.mtm order by group_num desc, ord asc limit $first_num, $scale";
}

try{
	$stmh = $pdo->query($sql);

	$sql = "select * from dnjstjq25.mtm";
	$stmh1 = $pdo->query($sql);
	...
	?>
<body>
<form name="board_form" method="post" action="mtm.php?mode=search">
	<div id="list_search">
...
while($row = $stmh->fetch(PDO::FETCH_ASSOC)){
	...
	<div id="list_item">
		...
		<div id="list_item2"><a href="mtm_view.php?num=<?=$item_num?>&page=<?=$page?>"><?=$item_subject?></a></div>
	...
	<?php
	 ...
	 if($page!=1 && $page>$page_scale)
	 {
	 	...
	 	if($prev_page <= 0)
	 		$prev_page = 1;
	 	print "<a href=mtm.php?page=$prev_page>◀ </a>";
	 }
	 ...
	 for($i=$start_page; $i<=$end_page && $i<= $total_page; $i++) 
      {        
        if($page==$i) 
           ...
           print "<a href=mtm.php?page=$i>[$i]</a>";
      }
      ...
      if($page<$total_page)
      {
        ...
        print "<a href=list2.php?page=$next_page> ▶</a><p>";
      }
      ?>
      </div>
  </div>
  <div id="write_button">
  	<a href="mtm.php?&page=<?=$page?>">목록</a>&nbsp;
  	<?php 
	 if(isset($_SESSION["uid"]))
	 {
	 		?>
	 		<a href="mtm_write.php?">글쓰기</a>
	 		<?php
	 }
	 ?>
	</div>
