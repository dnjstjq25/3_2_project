...
<head>
	...
	.menu-list-c:hover .sub-menu4{display: block;}
	 ...
	 .pay_result{margin-top: 15px; margin-left: 15px; border-bottom: 1px solid #4C4C4C; padding-bottom: 10px;}
...
<form name='buys_post' action='./buys_post.php' method='post' enctype='multipart/form-data'>
	...
	<input type='hidden' name='_count' value="<?=$count?>"> <!--수량-->
	
<div class = "wrap">
	<div class ="wrap2">		
		<?php
		try{
			$sql="select * from dnjstjq25.order_list where uid='$uid'";
			$stmh = $pdo->prepare($sql);  
		  	$stmh->bindValue(1, $uid, PDO::PARAM_STR);
			$stmh->execute();

			while ($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
				$item_reg_date  = $row["reg_date"];  
		   		$item_subject = str_replace(" ", "&nbsp;", $row["subject"]); 
		   		$item_totals   = $row["totals"];
		   		$item_file01  = $row["file01"];
			
			?>
			<div class="pay_result">
				<li>상품명 : <?=$item_subject ?>&nbsp; &nbsp; 결제일 : <?=$item_reg_date ?>&nbsp; &nbsp;총 결제액 : <?=number_format($item_totals) ?>원
			</div>
			<?php
		}
		?>
			<div class="info" align="center">
				<p>구매해주셔서 감사합니다. 오늘 하루도 행복한 일만 가득하시길..</p>
				<p id="info_pay">* 입금이 확인되면 문자 메세지로 안내 드립니다.</p>
				<p id="info_pay">* 배송이 시작되면 송장번호와 택배회사를 문자 메세지로 안내 드립니다.<p>
			</div>
			<?php
