...
<head>
	...
	<style>
		...
		.list_top_title{float: left; width: 100%; background: #ededed; margin-top: 10px; height: 50px; border-top: 2px solid #5d5d5d;}
		.list_bbs li{display: inline;}
		.li_subject{margin-left: 355px;}
		.li_id{margin-left: 540px;}
		.li_day{margin-left: 50px;}
		.li_hit{margin-left: 50px;}
		#list_item {float:left;height:20px;margin-top:4px;padding:6px;border-bottom:1px solid #eeeeee;}
		#list_item #list_item1{	float:left;	margin-left: 30px; width:40px;	text-align:center;}
		#list_item #list_item2{	float:left;	margin-left: 5px; width:735px;}
		#list_item #list_item3{	float:left;	margin-left: 185px; width:65px;	text-align:center;}
...
if($mode=="search"){
	...
	$sql="select * from dnjstjq25.greet1 where $find like '%search%' order by num desc";
} else {
	$sql="select * from dnjstjq25.greet1 order by num desc limit $first_num, $scale";
}

try{
	$stmh = $pdo->query($sql);

	$sql = "select * from dnjstjq25.greet1";
	$stmh1 = $pdo->query($sql);
	...
	?>
<body>
<form name="board_form" method="post" action="notice.php?mode=search">
	<div id="list_search">
...
while($row = $stmh->fetch(PDO::FETCH_ASSOC)){
	...
	<div id="list_item">
		...
		<div id="list_item2"><a href="qna_view.php?num=<?=$item_num?>&page=<?=page?>"><?=$item_subject?></a></div>
	...
	<?php
	 ...
	 if($page!=1 && $page>$page_scale)
	 {
	 	...
	 	if($prev_page <= 0)
	 		$prev_page = 1;
	 	print "<a href=qna.php?page=$prev_page>◀ </a>";
	 }
	 ...
	 for($i=$start_page; $i<=$end_page && $i<= $total_page; $i++) 
      {        
        if($page==$i) 
           ...
           print "<a href=qna.php?page=$i>[$i]</a>";
      }
      ...
      if($page<$total_page)
      {
        ...
        print "<a href=list1.php?page=$next_page> ▶</a><p>";
      }
      ?>
      </div>
  </div>
  <div id="write_button">
  	<a href="qna.php?&page=<?=$page?>">목록</a>&nbsp;
  	<?php 
	 if(isset($_SESSION["uid"]))
	 {
	 	if($_SESSION["uid"]=="admin"){
	 		?>
	 		<a href="qna_write.php?">글쓰기</a>
	 		<?php
	 	}
	 }
	 ?>
