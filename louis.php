...
<head>
	...
	.menu-list-c:hover .sub-menu4{display: block;}
	 ...
	 #view_title{margin-top: 20px; overflow: hidden; float:left; margin-left: 55px; text-align: center; width: 290px; }
...
<div class="menu-arrow"><a href="brand.php">브랜드</a> > <a href="louis.php">루이 비통</a></div>
<div class = "wrap">
	<div class ="wrap2">
		<?php
		 ...
		 try{
		     $sql = "select * from dnjstjq25.per_up where ctg='louis'";
		    ...
		    $item_price   = $row["price"]; //가격
		     $itme_no = $row["no"]; //번호
		    $item_ctg = $ctg["ctg"]; //브랜드 구분
		 ?>
        <div id="view_title">
	         <div id="view_content">
				<a href="per_detail.php?no=<?=$row[no]?>&ctg=<?=$row[ctg]?>"><img src="./data/<?=$row[file01]?>" width="290px" height="290px"></a>
	         </div>
		  		<?= $item_subject ?><br>
	         	₩<?= $item_price ?>
		</div>
